-- Schema Hardening for P0 Pickup System
...
-- 4. Clean up old test data (optional)
-- DELETE FROM orders WHERE status = 'pending' AND created_at < NOW() - INTERVAL '1 hour';
-- DELETE FROM widget_events WHERE type IN ('order_paid', 'order_picked_up') AND created_at < NOW() - INTERVAL '1 hour';
